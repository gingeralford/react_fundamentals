...
//actually just declaring a function, not a functional component
    ...
    useEffect(() => {
        if (queryNum !== ''){
            fetch(`http://numbersapi.com/${queryNum}`)
                .then(res => res.text()) //turning it into text instead of json on purpose? cause it will look better
                .then(json => { //it's ok that this is called json, because it can be named anything at the start of each then statement.
                    setResults(json);
                    console.log(json)
                })
        }
